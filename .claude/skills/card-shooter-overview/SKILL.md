...
### 3.3 友方实体 / 骷髅 / 召唤 vs 实体化（v8 强化）
`spawnSkeleton(world, opts)` → 创建 `_isSkeleton=true / kind='skeleton'` 的 `Bullet` 实例，挂 `EntityTurn` dash hook。
同样的 spawn 模板还有 `spawnSwordSaint`、`spawnUndeadDragon`。

所有"会 buff 骷髅"的卡（骷髅号角 / 爆骨花展露 / 叫魂钻 / 战旗 / 墓穴）必须**显式**遍历 `world.bullets` 或监听 `summonSpawned` 事件 — 不要靠 Hook 继承（已废弃模型）。

**两种友方单位的本质区别（必读，v8 踩过坑）**：

| 类别 | 来源 | 是否继承本回合 hooks（主卡 / 侧卡 effects） |
|---|---|---|
| **召唤**（spawnSkeleton / spawnSwordSaint / spawnUndeadDragon …） | 凭空生成的独立单位 | **不继承** — 行为完全由 spawnXxx 自己定义；不读 `world._turnHookCards` |
| **实体化子弹**（entityLayers>0 的子弹撞墙 / lifetime 耗尽后停在原地）| 那颗子弹本身变成单位 | **继承** — 单位 = 子弹，hooks 还在它身上 |

设计直觉：实体化是"子弹的延续"（注铅 +10 攻、爆骨花 buff 等在发射时已写到子弹上）；召唤是"凭空生成"，不应被本回合手牌组合影响。

新加召唤型卡的检查清单：
1. 不要 `copyHooksFrom` 任何模板
2. spawnXxx 内只用 `new Bullet(...)` + `addHook(自己定义的 EntityTurn / Destroyed)` + `activate(now)`
3. activate 内的 permUpgrades（永久升级）会应用 — 这是符合"永久升级对全友方生效"语义的，OK
4. 召唤的攻击：用 `applyAoe(world, b, {damage: b.attack, ...})` 直接结算；或走撞击流程但 hooks 为空

详细代码模板与防重入坑见 `modify-mechanic` skill §2.9。
...
### 3.9 增益 FX 系统（v8 起）— 不要手写 buff 浮字

游戏里所有"被 buff 的视觉反馈"通过两层自动系统：

**A. 炮台用卡 buff icon**（伤害+1 / 穿透+3 等飘字在炮台两侧）：
- 入口 `_emitCannonBuffFX(world, items)` —— **不要直接调**
- 用 `_computePerCardBuffItems(world, tpl, useList)` 算每张卡的 PreActive 贡献（dry 跑卡的 hooks 在临时 Bullet 上）
- 由 `fireFromCards.continueFire()` 自动调用
- 位置：炮台两侧 + 后方 180° 弧（excludes 前 ±90° 半圆，不挡炮口）
- 主卡 +1 + 副卡 +3 → 飘 2 条 ⚔+1 + ⚔+3（不合并为 +4）

**B. 实体 buff/loss 自动侦测**：
- 每帧 `_tickEntityBuffDiff(world)` 扫描所有友方 + `entityLayersMax>0` 的 Bullet
- 对比 `attack` / `entityLayers` 与上帧 snapshot
- ↑ 增加 → ⚔/🛡 增益浮字（在实体头上）
- ↓ 减少 → 灰色 🛡- 减损浮字
- **写新卡时只需 `b.attack += N` / `b.entityLayers += N` —— FX 自动出现**
- 不要在卡定义里手动 `_emitEntityBuffFX(...)`。已废弃模式：v8 前的 `_warBannerTier` / `_warDrumTier` 都手写过，现在统一移除

FX 配色对照（`_BUFF_FX` 字典）：
- atk ⚔ 红 / pen 🎯 青 / bnd 🏐 蓝 / cnt 🔫 金 / wav 〰 紫 / ent 🛡 淡蓝 / fire 🔥 橙
- ent_loss 🛡 灰（减号）

### 3.10 动态 desc（state-dependent text）
`def.desc` 默认是 `{zh, en}` 字典。v8 起也支持函数：
```js
desc: (card) => ({
  zh: `弃置此牌${threshold - (card._discardCount || 0)}次以召唤亡灵龙！`,
  en: `Discard ${threshold - (card._discardCount || 0)} more time(s) to summon Undead Dragon!`,
})
```

`Card.desc` getter 自动检测 typeof，函数则 `desc(this)` 调一次。

用途：勇闯龙巢的"弃置剩余次数实时显示"。任何"卡牌描述要随状态变化"的场景都走这个 pattern。改完后 emit `deckChanged` 让 UI 重渲。

### 3.11 卡牌 _alwaysFaceUp 标志（v8 起）
def 加 `alwaysFaceUp: true` → Card 构造时把 `this._alwaysFaceUp = true`，`_updateFaceUp()` 即使卡不在边缘也保持正面。

适用：必须持续展露才能稳定触发的卡（造访剑圣 钻级 "当此牌在手牌中始终为正面"）。

实现见 `Card constructor`（读 def.alwaysFaceUp）+ `CardDeck._updateFaceUp`（OR 进 `_foresightFaceUp` 判断）。
